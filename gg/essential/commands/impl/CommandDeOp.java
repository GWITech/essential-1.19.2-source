...
@Metadata(mv = { 1, 6, 0 }, k = 1, xi = 48, d1 = { """
                                                   ...
                                                   H\u0016¨\u0006\u000b""" }, d2 = { "Lgg/essential/commands/impl/CommandDeOp;", "Lgg/essential/commands/impl/CommandOpBase;", "()V", "apply", "", "uuid", "Ljava/util/UUID;", "username", "", "spsManager", "Lgg/essential/network/connectionmanager/sps/SPSManager;", "essential" })
public final class CommandDeOp extends CommandOpBase
{
    @NotNull
    public static final CommandDeOp INSTANCE;
    ...
    public void apply(@NotNull final UUID uuid, @NotNull final String username, @NotNull final SPSManager spsManager) {
        ...
        if (spsManager.getOppedPlayers().contains(uuid)) {
            ...
            spsManager.updateOppedPlayers(SetsKt.minus((Set)oppedPlayers, (Object)uuid), spsManager.isAllowCheats());
            MinecraftUtils.INSTANCE.sendMessage("Removed op from " + username);
        }
        else {
            MinecraftUtils.INSTANCE.sendMessage(username + " is not opped.");
        }
    }
    
    static {
        INSTANCE = new CommandDeOp();
    }
}
