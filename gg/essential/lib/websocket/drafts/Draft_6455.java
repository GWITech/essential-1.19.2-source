...
import java.nio.*;
import gg.essential.lib.websocket.protocols.*;
import org.slf4j.*;
import gg.essential.lib.websocket.extensions.*;
...
public class Draft_6455 extends Draft
{
    ...
    public HandshakeState acceptHandshakeAsServer(final ClientHandshake handshakedata) throws InvalidHandshakeException {
        ...
        final Iterator<IExtension> iterator = this.knownExtensions.iterator();
        if (iterator.hasNext()) {
            final IExtension knownExtension = iterator.next();
    ...
    public HandshakeState acceptHandshakeAsClient(final ClientHandshake request, final ServerHandshake response) throws InvalidHandshakeException {
        ...
        HandshakeState extensionState = HandshakeState.NOT_MATCHED;
        response.getFieldValue("Sec-WebSocket-Extensions");
        final Iterator<IExtension> iterator = this.knownExtensions.iterator();
        if (iterator.hasNext()) {
            final IExtension knownExtension = iterator.next();
            this.extension = knownExtension;
    ...
    public ClientHandshakeBuilder postProcessHandshakeRequestAsClient(final ClientHandshakeBuilder request) {
        ...
        for (final IExtension knownExtension : this.knownExtensions) {
            knownExtension.getProvidedExtensionAsClient();
            if (knownExtension.getProvidedExtensionAsClient().length() != 0) {
    ...
    private Framedata translateSingleFrame(final ByteBuffer buffer) throws IncompleteException, InvalidDataException {
        ...
        this.translateSingleFrameCheckPacketSize(maxpacketsize, 2);
        ...
        if (payloadlength < 0 || payloadlength > 125) {
            final TranslatedPayloadMetaData payloadData = this.translateSingleFramePayloadLength$31b95c0(buffer, optcode, payloadlength, maxpacketsize);
    ...
    private TranslatedPayloadMetaData translateSingleFramePayloadLength$31b95c0(final ByteBuffer buffer, final Opcode optcode, final int oldPayloadlength, final int maxpacketsize) throws InvalidFrameException, IncompleteException, LimitExceededException {
        ...
        if (payloadlength == 126) {
            realpacketsize += 2;
            this.translateSingleFrameCheckPacketSize(maxpacketsize, 4);
            final byte[] sizebytes = { 0, buffer.get(), buffer.get() };
            payloadlength = new BigInteger(sizebytes).intValue();
        }
        else {
            realpacketsize += 8;
            this.translateSingleFrameCheckPacketSize(maxpacketsize, 10);
    ...
    private void processFrameBinary(final WebSocketImpl webSocketImpl, final Framedata frame) {
        try {
            webSocketImpl.getWebSocketListener().onWebsocketMessage$1d6d21a6(frame.getPayloadData());
        }
    ...
    private void logRuntimeException(final WebSocketImpl webSocketImpl, final RuntimeException e) {
        ...
        webSocketImpl.getWebSocketListener().onWebsocketError$6d401871(e);
    }
    ...
    private void processFrameIsFin(final WebSocketImpl webSocketImpl, final Framedata frame) throws InvalidDataException {
        ...
        if (this.currentContinuousFrame.getOpcode() == Opcode.TEXT) {
            ...
            ((FramedataImpl1)this.currentContinuousFrame).isValid();
            try {
                webSocketImpl.getWebSocketListener().onWebsocketMessage$1d6d21a6(this.currentContinuousFrame.getPayloadData());
            }
            catch (final RuntimeException e) {
                this.logRuntimeException(webSocketImpl, e);
            }
